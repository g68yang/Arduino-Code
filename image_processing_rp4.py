...
def preprocess_image(image_path):
    ...
    if img is None:
        ...
        return None
    img = cv2.resize(img, (224, 224), interpolation=cv2.INTER_AREA)  # Resize to model input size
...
# Function to send data from Raspberry Pi to Arduino
def send_data_to_arduino(plant_name):

    try:
        while True:
            # data = "Hello from Raspberry Pi\n"  # Sample data to send
            data = plant_name + "\n"
            sdata.write(data.encode('utf-8'))  # Encode and send data
            print("Sent to Arduino: " + data.strip())
...
send_data_to_arduino(plant_name)
